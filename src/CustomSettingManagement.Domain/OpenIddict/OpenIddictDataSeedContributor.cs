...
namespace CustomSettingManagement.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("CustomSettingManagement") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "CustomSettingManagement", DisplayName = "CustomSettingManagement API", Resources = { "CustomSettingManagement" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        var commonScopes = new List<string> {
            ...
            "CustomSettingManagement"
        };

        var configurationSection = _configuration.GetSection("OpenIddict:Applications");

        //Web Client
        var webClientId = configurationSection["CustomSettingManagement_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["CustomSettingManagement_Web:RootUrl"]!.EnsureEndsWith('/');

            /* CustomSettingManagement_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["CustomSettingManagement_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["CustomSettingManagement_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["CustomSettingManagement_Swagger:RootUrl"]?.TrimEnd('/');
